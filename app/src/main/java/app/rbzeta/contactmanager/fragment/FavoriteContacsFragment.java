...
public class FavoriteContacsFragment extends Fragment implements IContactListView,
    ...
    public static FavoriteContacsFragment getInstance(Bundle bundle){
        FavoriteContacsFragment frag = new FavoriteContacsFragment();
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View view = inflater.inflate(R.layout.content_contact_favorite,container,false);

        textNoContactFound = (TextView)view.findViewById(R.id.text_no_contact_found);
        mRecyclerView = (RecyclerView)view.findViewById(R.id.recycler_view_contact_list_fav);
        ...
        if (requestCode == REQUEST_CODE_NEW_CONTACT){
            if (resultCode == RESULT_OK){
                if (data.getExtras().getBoolean(ContactAddNewActivity.MSG_KEY_ADD_CONTACT_RESULT)){
                    mContactList.clear();
                    mAdapter.notifyDataSetChanged();
                    ContactListPresenter presenter = new ContactListPresenterImpl(this);
